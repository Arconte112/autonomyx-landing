...
- El `lastmod` del sitemap debe avanzar cuando cambie materialmente el contenido público. La revisión del posicionamiento actualizó `public/sitemap.xml` al 1 de agosto de 2026.
...
- La antigua cuadrícula de oportunidades fue reemplazada por un carrusel de industrias con navegación centrada y avance automático: manufactura, servicio al cliente, farmacéutica, logística, servicios financieros, energía y legal.
...
- Debajo de la lista `De la oportunidad a produccion` aparece un segundo manifiesto editorial: `Nuestro software te muestra exactamente la informacion necesaria para tomar tu proxima decision.` Las frases `exactamente la informacion necesaria` y `proxima decision` reciben el enfasis negro; el resto se mantiene gris y la version inglesa conserva el mismo significado y jerarquia.
...
- El carrusel de industrias usa siete escenas editoriales propias en `public/media/industry-*.webp`. Las tarjetas son panoramicas, muestran el nombre de la industria sin numeracion, una promesa breve y una descripcion concisa.
